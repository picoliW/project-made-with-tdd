...
test('Deve criar usuário via signup', () => {
  return request(app)
    .post('/auth/signup')
    .send({ name: 'Walter', mail: `${Date.now()}@mail.com`, passwd: '123' })
    .then((res) => {
...
test('Deve receber token ao logar', () => {
  ...
  return app.services.user
    ...
    .then(() => request(app).post('/auth/signin').send({ mail, passwd: '123' }))
    .then((res) => {
...
test('Não deve autenticar usuário com senha errada', () => {
  ...
  return app.services.user
    ...
    .then(() => request(app).post('/auth/signin').send({ mail, passwd: '321' }))
    .then((res) => {
